...
3. **HTTPS в Docker:** в каталог **`ssl/`** положите **`catalogbooks.crt`** и **`catalogbooks.key`** (см. [`ssl/README.md`](ssl/README.md)). Nginx на **80** отдаёт **301** на **`https://$host$request_uri`**, приложение — на **443**. В **`.env`** укажите **`APP_URL=https://…`** для вашего домена.
